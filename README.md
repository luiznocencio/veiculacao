...
Confere automaticamente se os comprovantes em PDF de cada site cobrem todos os dias do mês selecionado, com o banner correto no ar e a data legível — usando visão computacional (Anthropic Claude Haiku 4.5) por trás de uma função serverless.
...
- `api/analyze.js` — função serverless da Vercel. Recebe as imagens do frontend, chama a Anthropic com a chave guardada em `ANTHROPIC_API_KEY` (variável de ambiente do servidor) e devolve o resultado já processado. A chave nunca é exposta ao navegador.
...
4. **Settings → Environment Variables** → adicione `ANTHROPIC_API_KEY` com sua chave da Anthropic (marque Production, Preview e Development).
